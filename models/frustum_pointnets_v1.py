...
def get_instance_seg_v1_net(point_cloud, one_hot_vec,
    ...
    net = tf.expand_dims(point_cloud, 2)  # B×N×1×4

    # MLP(64,64)
    net = tf_util.conv2d(net, 64, [1, 1],
                         padding='VALID', stride=[1, 1],
                         bn=True, is_training=is_training,
                         scope='conv1', bn_decay=bn_decay)  # B×N×1×64
    net = tf_util.conv2d(net, 64, [1, 1],
                         padding='VALID', stride=[1, 1],
                         bn=True, is_training=is_training,
                         scope='conv2', bn_decay=bn_decay)  # B×N×1×64
    # MLP(64,128,1024)
    point_feat = tf_util.conv2d(net, 64, [1, 1],
                                padding='VALID', stride=[1, 1],
                                bn=True, is_training=is_training,
                                scope='conv3', bn_decay=bn_decay)  # B×N×1×64
    net = tf_util.conv2d(point_feat, 128, [1, 1],
                         padding='VALID', stride=[1, 1],
                         bn=True, is_training=is_training,
                         scope='conv4', bn_decay=bn_decay)  # B×N×1×128
    net = tf_util.conv2d(net, 1024, [1, 1],
                         padding='VALID', stride=[1, 1],
                         bn=True, is_training=is_training,
                         scope='conv5', bn_decay=bn_decay)  # B×N×1×1024
    # maxpool
    global_feat = tf_util.max_pool2d(net, [num_point, 1],
                                     padding='VALID', scope='maxpool')  # B×1×1×1024
    # tf.concat拼接张量，one_hot_vec(B,3)拓展成(B,1,1,3)，然后与global_feat在第3维拼接，得到(B,1,1,1027)
    global_feat = tf.concat([global_feat, tf.expand_dims(tf.expand_dims(one_hot_vec, 1), 1)], axis=3)
    # tf.tile复制平铺，得到(B,N,1,1027)
    global_feat_expand = tf.tile(global_feat, [1, num_point, 1, 1])
    # 拼接point_feat和global_feat_expand，得到(B,N,1,1091)
    ...
    net = tf_util.conv2d(concat_feat, 512, [1, 1],
                         padding='VALID', stride=[1, 1],
                         bn=True, is_training=is_training,
                         scope='conv6', bn_decay=bn_decay)  # B×N×1×512
    net = tf_util.conv2d(net, 256, [1, 1],
                         padding='VALID', stride=[1, 1],
                         bn=True, is_training=is_training,
                         scope='conv7', bn_decay=bn_decay)  # B×N×1×256
    net = tf_util.conv2d(net, 128, [1, 1],
                         padding='VALID', stride=[1, 1],
                         bn=True, is_training=is_training,
                         scope='conv8', bn_decay=bn_decay)  # B×N×1×128
    net = tf_util.conv2d(net, 128, [1, 1],
                         padding='VALID', stride=[1, 1],
                         bn=True, is_training=is_training,
                         scope='conv9', bn_decay=bn_decay)  # B×N×1×128
    # dropout
    net = tf_util.dropout(net, is_training, 'dp1', keep_prob=0.5)

    logits = tf_util.conv2d(net, 2, [1, 1],
                            padding='VALID', stride=[1, 1], activation_fn=None,
                            scope='conv10')  # B×N×1×2
    logits = tf.squeeze(logits, [2])  # B×N×2
