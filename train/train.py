...
if not os.path.exists(LOG_DIR): os.mkdir(LOG_DIR)   # log目录不存在，则创建一个
os.system('cp %s %s' % (MODEL_FILE, LOG_DIR))       # 复制模型文件到log目录
os.system('cp %s %s' % (os.path.join(BASE_DIR, 'train.py'), LOG_DIR))  # 复制train文件到log目录
LOG_FOUT = open(os.path.join(LOG_DIR, 'log_train.txt'), 'w')  # 打开训练参数记录文件
...
# staircase=False,连续更新即每步都会更新，staircase=True,离散更新即每decay_steps更新一次（*decay_rate）
def get_learning_rate(batch):
    learning_rate = tf.train.exponential_decay(  # 对学习率实行指数衰减
        BASE_LEARNING_RATE,     # 初始学习率
        batch * BATCH_SIZE,     # 当前step
        DECAY_STEP,             # 衰减step
        DECAY_RATE,             # 衰减率
        staircase=True)         # 离散更新
    learning_rate = tf.maximum(learning_rate, 0.00001)  # 学习率最小0.00001
    return learning_rate


# 获取移动平均权重，从0.5一直最大到0.99
def get_bn_decay(batch):
    bn_momentum = tf.train.exponential_decay(
        ...
        BN_DECAY_DECAY_RATE,
        staircase=True)
    bn_decay = tf.minimum(BN_DECAY_CLIP, 1 - bn_momentum)  # bn_decay最大0.99
    return bn_decay
...
def train():
    ...
    with tf.Graph().as_default():
        with tf.device('/gpu:' + str(GPU_INDEX)):
            ...
            train_op = optimizer.minimize(loss, global_step=batch)  # 更新网络权值，最小化损失函数，全局步batch+1
        ...
        for epoch in range(MAX_EPOCH):  # 训练、测试
...
def train_one_epoch(sess, ops, train_writer):
    ...
    '''
    is_training = True
    log_string(str(datetime.now()))     # 记录时间
